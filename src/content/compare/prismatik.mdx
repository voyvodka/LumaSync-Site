...
- If you're using a generic CH340/FT232 USB kit, the **strip and controller board are reusable**. Either reflash the microcontroller to the [LumaSync v1 serial frame](/docs/usb-leds/serial-protocol), or enable the v1.4+ **Adalight profile** in Settings → Device and keep the existing Adalight firmware. Then stop Prismatik, disconnect its USB port, start LumaSync, run the device health check. See [USB controllers](/docs/usb-leds/controllers).
...
- Your edge counts and calibration settings don't auto-import. Re-enter them in [Calibration](/docs/usb-leds/calibration).
