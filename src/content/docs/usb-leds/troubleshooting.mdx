...
1. **Driver not installed** — CH340 / CH341 on Windows sometimes need a manual driver from WCH's website. Install, reboot, check again.
...
4. **Unsupported chipset** — if the device enumerates but LumaSync doesn't recognise it, see [USB controllers → "My controller isn't on the list"](/docs/usb-leds/controllers#my-controller-isnt-on-the-list).

## `PORT_UNSUPPORTED` on connect

**Symptom**: You select a port, click Connect, and get `PORT_UNSUPPORTED` instead of opening.

From v1.5.2, LumaSync rejects non-USB serial endpoints up-front rather than letting the OS open them and silently swallowing writes. Common offenders:

- `/dev/cu.Bluetooth-Incoming-Port` and other Bluetooth virtual ports on macOS — they enumerate as serial but don't go anywhere.
- Motherboard / PCI / Unknown-class serial ports listed by the OS but not actually connected to a USB endpoint.

The fix is straightforward: pick the actual USB controller from the dropdown. The auto-detected list is filtered to USB endpoints only, so this error only appears when you've manually pasted a non-USB path.
...
## App won't launch at all on macOS

If you're on 1.5.2 and the app crashes on startup with a `dyld: Library not loaded: @rpath/libswift_Concurrency.dylib` error (it affected Macs without Xcode installed), update to v1.5.3 — see [Install → macOS](/docs/getting-started/install/#macos).
