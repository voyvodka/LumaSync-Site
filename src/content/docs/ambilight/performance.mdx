...
| Linux X11                        | 15–30 ms  | 60 fps    | The supported Linux path       |

**Wayland is not a supported capture path**, so there are no figures for it. LumaSync captures X11 on Linux; Wayland needs `xdg-desktop-portal` support that is not built. A Wayland session may appear to work — the capture library falls through to PipeWire — but that path is untested and unsupported, and its performance is not something we can characterise.

For the Hue path specifically, **Σ caps at 20 fps** regardless of what your capture pipeline delivers. The bridge is the bottleneck. USB LEDs can go up to 60 fps — but only on short strips.

### USB frame rate vs LED count

From v1.4, LumaSync adapts the USB frame rate to your LED count so every frame fits inside the 115200-baud budget. You don't configure this — it's computed automatically — but the ceiling is worth knowing:
...
Beyond 200 LEDs the serial link becomes the bottleneck. Split the strip across two controllers if you need higher frame rate at that scale.
...
- **On Linux, high Δ under a Wayland session**: LumaSync supports X11 capture only. Log in to an X11 session; a Wayland one is running through an untested fallback.
...
- **Gamma curves**. The LEDs might drive the right _numbers_ within the measured latency but look visually wrong because the strip's gamma isn't calibrated. Telemetry says "fast"; eyes say "off".
