...
## 2026-05-18 - Keyboard Shortcuts Accessibility

**Learning:** When using standard HTML `<kbd>` elements to display keyboard shortcuts (e.g., `⌘K`), screen readers often read out the symbols confusingly or redundantly. **Action:** Always add `aria-keyshortcuts` to the parent interactive element (button or link) to define the shortcut semantically (e.g., `aria-keyshortcuts="Meta+K Control+K"`). Simultaneously, wrap the visual `<kbd>` elements in a `<span>` or `<kbd>` wrapper with `aria-hidden="true"` so that the screen reader correctly announces the standardized shortcut via `aria-keyshortcuts` instead of parsing the raw symbol characters visually presented on screen.
