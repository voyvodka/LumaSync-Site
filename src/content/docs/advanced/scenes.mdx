...
**User-named custom presets (saving your own) are not available in {LATEST_VERSION}. The five built-in tiles are the complete scene catalog.** If you want a "movie mode" colour you like better than the default amber, the workflow is:
...
Scenes are **static colour**. They don't sample the screen. If you want colour that tracks on-screen content, that's Ambilight mode — see [Screen capture](/docs/ambilight/screen-capture) and [Ambilight tuning](/docs/ambilight/tuning).
