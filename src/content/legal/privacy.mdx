...
Umami is enabled only when both environment variables (`PUBLIC_UMAMI_SITE_ID` + `PUBLIC_UMAMI_SRC`) are populated at build time. Local development builds do not emit the tracking script.
