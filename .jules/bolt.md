...
## 2026-05-23 - Flattening array operations in Astro builds\n\n**Learning:** Chained array operations like `.reduce` followed by `.flatMap` (containing `.filter` and `.map`) during Astro SSR/SSG processes loop over the same collections multiple times, allocating intermediate arrays. While unmeasurable on small collections (<50 items), it adds unnecessary overhead to page generation.\n**Action:** To improve performance during Astro SSG/SSR builds, consolidate multiple sequential array passes into a single `.reduce()` operation with an inner manual loop. This eliminates redundant iterations and intermediate array allocations.
