...
**Learning:** Transient interactive elements that use CSS transforms to become visible, such as a 'Skip to content' link that moves on screen via `translateY`, must still include standard focus rings (`outline`) and tactile `:active` scale states. These states provide crucial visual feedback to keyboard users who navigate to the element and mouse/touch users who might click it when it is visible. **Action:** When creating skip links or similar transient interactive elements, always include `outline: 2px solid var(--focus-ring);` on `:focus-visible` and `transform: scale(0.96);` on `:active`, ensuring the active state is wrapped in `@media (prefers-reduced-motion: no-preference)`.
